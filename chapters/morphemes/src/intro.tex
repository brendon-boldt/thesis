...
Ultimately, such studies form one of the pillars of emergent communication research: learning what emergent language can tell us about human language (\cref{ch:review}).
...
\citet{narasimhan-etal-2015-unsupervised} introduce a semi-supervised segment induction algorithm that uses semantic features to guide segmentation (viz.\@ groups of morphologically related words), although meanings are not modeled explicitly and ``word'' is not a well-defined concept for emergent languages.
...
\phantom{}\cmt{revise}
The task of morpheme induction forms the basis of a broader process of ``language acquisition'' for emergent language.
Starting with the tasks of morpheme induction aligns this work with a broadly bottom-up, structuralist approach to linguistics insofar as we are looking at trying to discover low-level meaningful units which can later composed into more more complex structures.
This is contrast to other approaches to explaining language acquisition that emphasize top-down structures, highlighting the importance of context in determining meaning.
In the case of emergent language, we would argue that a bottom-approach is more appropriate, at least initially, when the presence and nature of meaningful structures in these emergent communication protocols are, as yet, unknown.
This is because a bottom-up approach makes the fewest assumptions about the nature of form and meaning in emergent language, which has been shown to have structure which diverge from that of human language \citep{kottur2017natural,lake2018generalization,chaabouni2019antiefficient}.
Furthermore, interpretability in deep learning more broadly entails explaining high level behavior in terms of the interactions of low-level parts (e.g., explaining the interpretation of utterances in terms of minimal pairs of form and meaning).
