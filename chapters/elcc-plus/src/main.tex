...
The first environment is the discrimination variant the signaling game where the observations are concatenations of one-hot vectors.
...
In \Cref{fig:serialization}, we present a simplified serialization of episodes in the signaling game and navigation environments.
For the signaling game, the serialization is relatively straightforward since every episode consists of a single timestep with the same components;
...
The observations themselves have more information and structure compared to the single vector in the signaling game.
...
The signaling game only consists trivially of one or two steps, and it is much more natural to represent as not having any steps at all.
...
The interface implementation for the signaling game is, again, relatively straightforward due to the simplicity of the environment and, in turn, serialization.
