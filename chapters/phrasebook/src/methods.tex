...
We use the CSAR algorithm to induce morphemes from the annotated emergent language corpora produced by the emergent language game (\cref{ch:morphemes}).
Its outline is:
...
Unlike the sender, the form is not subtracted from the message, and the algorithm proceeds through the whole phrasebook only one time;
  thus, a sort of superposition of all matching morphemes is considered.
