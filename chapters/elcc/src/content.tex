...
\begin{table}
  ...
  \begin{tabular}{lllccr}
    ...
    \midrule
    % \citet{kharitonov-etal-2019-egg}, discrim.         & signaling   & synthetic & $3$ \\
    \citet{kharitonov-etal-2019-egg}                       & signaling   & synthetic & No & No & $15$ \\
    \citet{yao2022linking}            & signaling   & natural   & No & No & $2$ \\
    \citet{mu2021generalizations}     & signaling   & both      & No & No & $6$ \\
    \citet{chaabouni2022emergent}     & signaling   & natural   & Yes & No & $5$ \\
...
We separate the discussion of particular systems into two subsections: signaling games (\Cref{elcc:sec:signaling}) and its variations which represent a large proportion of system discussed in the literature and other games (\Cref{elcc:sec:other-games}) which go beyond the standard signaling framework.


\subsection{Scope}
The scope of the contents of \theLib{} is largely the same as discussed in reviews such as \citet{lazaridou2020review} and \cref{sec:rev-scope}.
...
\unskip\label{elcc:sec:signaling}
The \emph{signaling game} (or \emph{reference game}) \citep{lewis1970ConventionAP} represents a plurality, if not majority, of the systems present in the literature.
A brief, non-exhaustive review of the literature yielded $43$ papers which use minor variations of the signaling game, a large number considering the modest body of emergent communication literature (see \Cref{elcc:sec:sg-list}).
...
The basic format of the signaling game is a single round of the \emph{sender} agent making an observation, passing a message to the \emph{receiver} agent, and the receiver performing an action based on the information from the message.
...
The two major subtypes of the signaling game are the \emph{discrimination game} and the \emph{reconstruction game}.
...
\paragraph{Vanilla}
For the most basic form of the signaling game, which we term ``vanilla'', we use the implementation provided in the Emergence of lanGuage in Games (EGG) framework \citep[MIT license]{kharitonov-etal-2019-egg}.
It is vanilla insofar as it comprises the signaling game with the simplest possible observations (synthetic, concatenated one-hot vectors), a standard agent architecture (i.e., RNNs), and no additional dynamics or variations on the game.
Both the discrimination game and the reconstruction game are included.
This system provides a good point of comparison for other ECSs which introduce variations on the signaling game.
...
``Linking emergent and natural languages via corpus transfer'' \citep[MIT license]{yao2022linking} presents a variant of the signaling game which uses embeddings of natural images as the observations.
...
``Emergent communication of generalizations'' \citep[MIT license]{mu2021generalizations} presents a variant of the discrimination signaling game which they term the \emph{concept game}.
...
``Emergent communication at scale'' \citep[Apache 2.0-license]{chaabouni2022emergent} presents a signaling game system with populations of agents instead of the standard fixed pair of sender and receiver.
...
Considering that the signaling game is close to the simplest possible game for an ECS, moving beyond the signaling game generally entails an increase in complexity.
...
``Generalizing Emergent Communication'' \citep[BSD-3-clause license]{unger2020GeneralizingEC} introduces an ECS which takes some of the basic structure of the signaling game and applies it to a navigation-based system derived from the synthetic Minigrid/BabyAI environment \citep{chevalier2018babyai,MinigridMiniworld23}.
...
What distinguishes this system most from the signaling game is that it is multi-step and embodied such that the  utterances within an episodes are dependent on each other.
...
These additional features in the system add the potential for communication strategies that are shaped by a variety of heterogeneous factors rather than simply the distribution of observations (as in the signaling game).
