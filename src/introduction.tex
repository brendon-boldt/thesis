...
%%%%%%%%%%%%%%%%%%%
The year is 2016.
AlphaGo defeats Lee Sedol, reigning Go world champion \cmt{check}.
OpenAI is making a name for itself as reinforcement learning lab with OpenAI Gym \cmt{cite}.
...
\cmt{History of deep learning \url{https://arxiv.org/pdf/1701.07274}}
AlphaZero \cmt{cite} is on the horizon, making a powerful argument that hand-coded algorithm are inferior to deep neural net-powered searching.
...
Long had expert-designed systems reigned in games like chess, starting with Deep Blue \cmt{cite} in \cmt{year} culminating \cmt{wc} Stockfish \cmt{cite}, but AlphaZero, with almost no domain knowledge, dealt a mighty blow to the paradigms dominance.

It is here we begin the analogy with subject matter of this thesis:
The origins of human language and its evolution had for decades been studied through expert-designed simulations \cmt{citations} (i.e., \emph{emergent language}), but perhaps deep learning-driven exploration with reinforcement learning could revolutionize the study of language evolution like it had done for the aforementioned applications.
In this vein papers such as \cmt{seminal el papers: foerster, laz2017, havrylov} came on to the scene to being the work of harnessing deep RL for emergent language.

One way to look at scientific exploration through simulation is searching for the ``interesting'' interface between order and chaos much like the boundary region of the Mandelbrot set \cmt{cite}.
...
While the classical approaches to emergent language start with order and introduce chaos to find the interesting, deep learning-based emergent language begins in the chaos: design the environment and let multi-agent reinforcement learning work its magic, hopefully the order there.
...
In the seven or so years of deep learning-based emergent communication's existence, there has been little measurable progress toward the farther-reaching applications of the field.
Much of the research has focused on addressing small-scale, isolated phenomena without a clear way to unify the findings into a broader understanding.
...
Furthermore, despite the potential for groundbreaking applications in natural language processing and linguistics, emergent communication has not yielded any broadly-applicable contributions either \citep{boldt2024review}.
...
Lastly, \cref{ch:morphemes,ch:phrasebook} introduce methods for detecting linguistic structures in emergent language corpora.
