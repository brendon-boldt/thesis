...
In the last couple years,
  ...
  OpenAI has been making a name for itself as a reinforcement learning lab with OpenAI Gym \citep{openaigym} and OpenAI Five \citep{openai2019dota2largescale}.
...
It is in this milieu that I began to develop the ideas that would grow into this thesis.
...
The origins of human language and its evolution had for decades been studied through expert-designed simulations based on theoretically-motivated models (i.e., \emph{emergent language}) \citep{kirby2002alife}, but perhaps deep reinforcement learning could revolutionize the study of language evolution like it had other problems many a time before.
Just as AlphaZero had re-derived many chess strategies based only the rules of the game (as well as finding some new ones), maybe similar techniques could rederive the communication strategies which characterize human language?
...
Since this process of language emerging is far more analogous to how human language develops and is learned, it has a much greater potential to contribute to the scientific understanding of human language.
...
This intersection between linguistics and computer science gained momentum in the 1990s with the advances in computational power enabling the use of \emph{agent-based modeling} for simulations of communication systems \citep{steels1997synthetic}.
...
Deep learning-based emergent language (DEL)---the body of literature in which this thesis is most directly situated---it is the rebellious child of CEL and the deep reinforcement learning boom of the late 2010s (cf.\@ \hyperref[ch:prologue]{the Prologue}).
...
I say rebellious here because, rather than being a harmonious union of the two,
  ...
  and (2) DEL diverges from CEL by a relative absence of grounding in the broader body linguistics scholarship, often starting nearly from scratch in the formulation of hypotheses and experimental approaches.
    Furthermore, translating any such formal models from CEL into DEL is made difficult by the complexity of deep learning approaches.
...
The resulting behavior, most especially the communication protocol, is typically the object of analysis, addressing question such as:
  ...
  Do these features align at all with human language?
...
The methods of classical emergent language, for the most part, assume that the basic structures in the emergent languages (e.g., words) are already evident,
  meaning that the project of finding structure in deep emergent languages is, in a way, trying to recover some of what was lost in the classical-to-deep transition and better connect the two sides of the broader field.
...
Thus, this thesis introduces goal-oriented (\cref{sec:intro-review}), environment-agnostic (\cref{sec:intro-elcc}) analytical algorithms to support measurable progress towards both the engineering-centric (\cref{sec:intro-xferbench}) and scientific (\cref{sec:intro-morph}) goals of emergent language.
...
Thus this chapter provides both the motivation and scaffold for the \emph{goal-oriented} analytical algorithms introduced the later chapters.
...
Furthermore, making the corpora available in a simple JSON-based format (and obviating running the environments in the first place) lowers the barrier of entry to studying emergent languages in general, especially for those without a background in computing.
...
XferBench is \emph{goal-oriented} insofar as it is directly measuring how well emergent languages function in place of human languages in deep learning settings, a major component of the \emph{engineering} goals of emergent language.
...
The patterns observed in synthesizing and analyzing emergent language at the morpheme-level present one of the most principled investigations of the syntax of deep learning-based emergent languages to date.
